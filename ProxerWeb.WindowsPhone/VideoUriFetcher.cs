...
namespace Proxer
{
    public static class VideoUriFetcher
    {
        ...
        private static async Task<Uri> GetDailymotionStreamUri(Uri baseUri)
        {
            string lResponse = await HttpUtility.GetRequest(baseUri);
            Match lFirstMatch = new Regex("\"(http[^\"]+?\\.mp4\\S+?)\"").Match(lResponse);
            if (lFirstMatch.Success) return new Uri(lFirstMatch.Captures[0].Value);
            throw new Exception();
        }

        private static async Task<Uri> GetMp4UploadStreamUri(Uri baseUri)
        {
            string lResponse = await HttpUtility.GetRequest(baseUri);
        ...
        private static async Task<Uri> GetProxerStreamUri(Uri baseUri)
        {
            string lResponse = await HttpUtility.GetRequest(baseUri);
        ...
        private static async Task<Uri> GetStreamcloudStreamUri(Uri baseUri)
        {
            string lResponse = await HttpUtility.GetRequest(baseUri);
            ...
            string lPostResponse = await HttpUtility.PostRequest(baseUri, lPostArgsDictionary);
        ...
        public static async Task HandleStreamPartnerUri(Uri baseUri)
        {
            switch (baseUri.Host.Replace("www.", string.Empty))
            {
                case "stream.proxer.me":
                    StartVideoFromUri(await GetProxerStreamUri(baseUri));
                    break;
                case "mp4upload.com":
                    StartVideoFromUri(await GetMp4UploadStreamUri(baseUri));
                    break;
                case "streamcloud.eu":
                    StartVideoFromUri(await GetStreamcloudStreamUri(baseUri));
                    break;
                case "dailymotion.com":
                    StartVideoFromUri(await GetDailymotionStreamUri(baseUri));
                    break;
                default:
